...
public class AltaTratamientoDialog extends JDialog {
	...
	public static void main(String[] args) {
		try {
			AltaTratamientoDialog dialog = new AltaTratamientoDialog();
	...
	public AltaTratamientoDialog() throws SQLException {
		...
		JLabel lblProv = new JLabel("ALTA TRATAMIENTO");
		lblProv.setFont(new Font("SansSerif", Font.BOLD, 29));
		lblProv.setBounds(228, 32, 301, 38);
		...
		cmbStock.setModel(rellenarDatos("tratamiento", "nombre", modeloDatos));
		...
		contentPanel.add(btnAceptar);
		{
			...
			lblFondo.setIcon(new ImageIcon(AltaTratamientoDialog.class.getResource("/fotos/baja_alta.png")));
		...
		btnAceptar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				String id;
				int op = JOptionPane.showConfirmDialog(null, "¿Seguro que quiere dar de alta el tratamiento " + cmbStock.getSelectedItem() + "?", "Confirmar baja", JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE);
				if(op == 0) {
					try {
						con.cambiarAlta(seleccionado,"tratamiento","idtratamiento");
						...
						JOptionPane.showMessageDialog(null, "Error al dar de Alta al tratamiento",
